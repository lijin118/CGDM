...
args.train_path = "dataset/office/%s/images" % source
args.val_path = "dataset/office/%s/images" % traget
...
def train(num_epoch):
    ...
    for ep in range(num_epoch):
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            entropy_loss = Entropy_both(output_t1)
            entropy_loss += Entropy_both(output_t2)
            #print("entropy_size:",entropy_loss)
            ...
            loss_dis = discrepancy_matrix(output_t1,output_t2)

            all_loss = loss1 + loss2 - 0.01 * loss_dis + 0.01 * entropy_loss
            all_loss.backward()
            optimizer_f.step()
            ...
            for i in range(num_k):
                ...
                all_loss = 0.01 * loss_dis + 0.01 * entropy_loss
