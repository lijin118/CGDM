...
def train(num_epoch):
    ...
    #CrossEntropyLabelSmooth(num_classes = 12, epsilon=0.1).cuda()
    for ep in range(num_epoch):
        since = time.time()
        for batch_idx, data in enumerate(dataset):
            if ep > 10 and batch_idx % args.pseudo_interval == 0:
                ...
                #print("Obtaining target label finished.")
            G.train()
            F1.train()
            F2.train()
            ...
            if ep > 10:
                supervision_loss = criterion(output_t1, pseudo_label_t) + criterion(output_t2, pseudo_label_t)
            else:
                supervision_loss = 0
            ...
            for i in range(num_k):
                ...
                #print(pseudo_label_t)
                if ep > 10:
                    gmn_loss = gradient_mathing_loss(args, output_s1,output_s2, label_s, output_t1, output_t2, pseudo_label_t, G, F1, F2)
                else: 
                    gmn_loss = 0
